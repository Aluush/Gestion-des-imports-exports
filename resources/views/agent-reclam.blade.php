...
<body> 
  ...
  <div class="content">
    <div class="row">
     ...
     @endif
          <form class="trier" method="post" action="{{route('agent-reclame-un')}}">
            @csrf
            <div class="row">
              <div class="col-3 m-2"> 
                @if ($errors->any ())
              ...
              <div class="col-3 m-2">  
                <span>Etat</span>
                <select class="form-control w-100" name="etat">
                    <option value="">Sélectionner une état</option>
          ...
          <table class="table tab">
            ...
            <tbody>
              ...
              <tr>
                <td>{{$reclam->rec_id}}</td>
                <td>{{$reclam->email}}</td>
                <td>{{$reclam->prenom_gerant}} {{$reclam->nom_gerant}}</td>
                <td>{{$reclam->{'op/trans'} }}</td>
                <td>{{$reclam->theme}}</td>
                <td> <span class="{{$reclam->etat}}">{{$reclam->etat}}</span> </td>
                <td><button><a href="{{ route('show_reclam_complete', ['id' => $reclam->rec_id]) }}">Affecter</a></button></td>
              </tr>
            @endforeach
